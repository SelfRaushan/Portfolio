...
const Contact = () => {
  ...
  const handleSubmit = e => {
    ...
    const subject = encodeURIComponent(`Contact from ${form.name}: ${form.reason}`);
    const body = encodeURIComponent(`Hello Raushan,\n\nMy name is ${form.name} and my email is ${form.email}.\n\nMy reason for contacting you is: ${form.reason}\n\nMessage:\n${form.message}`);
    
    window.location.href = `mailto:${CONTACT_EMAIL}?subject=${subject}&body=${body}`;
    
    setStatus("Redirecting to your email client...");
    setTimeout(() => {
        setStatus("Your message is ready to be sent!");
